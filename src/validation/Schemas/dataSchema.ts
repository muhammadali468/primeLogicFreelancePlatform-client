import type { UserLoginTypes, UserRegisterTypes, UserUpdateTypes } from "@/types";
...
export const updateSchema: ZodType<UserUpdateTypes> = object({
  ...
  email: string().email().min(5, { message: "This field requires at least 5 characters" }).max(100, { message: "Email is too long" }).toLowerCase()
});
